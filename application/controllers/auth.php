...
class Auth extends CI_Controller
{
    ...
    **/
    function __construct()
    {
        ...
        if ($this->ion_auth->logged_in()) {
            $this->template->set('title', 'Autenticação');
            $this->template->set('nav', 'auth');
    ...
    **/
    function index()
    {
        if (!$this->ion_auth->logged_in()) {
            ...
            //redirect them to the home page because
            //they must be an administrator to view this
    ...
    **/
    function login()
    {
        ...
        $this->form_validation->set_rules(
            'identity', 'Identity', 'trim|required|xss_clean'
        );
        $this->form_validation->set_rules(
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login(
                ...
                redirect('auth/login', 'refresh');
                //use redirects instead of loading views for
                //compatibility with MY_Controller libraries
            }
        } else {
    ...
    **/
    function changePassword()
    {
        ...
        $this->form_validation->set_rules(
            'new_confirm',
            $this->lang
                ->line('change_password_validation_new_password_confirm_label'),
            'required'
        );
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->data['min_password_length'] = $this
                ->config->item('min_password_length', 'ion_auth');
            ...
            //render
            $this->_render_page('auth/changePassword', $this->data);
        } else {
            $identity = $this->session->userdata('identity');

            $change = $this->ion_auth->changePassword($identity, $this->input->post('old'), $this->input->post('new'));
    ...
    **/
    ...
    /**@
    Função para fazer reset à password
    ultimo passo para recuperar a password
    @code nao sei bem o que é
    @return void
    **/
        ...
        if ($user) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($this->_valid_csrf_nonce() === false || $user->id != $this->input->post('user_id')) {
                    ...
                    $change = $this->ion_auth->resetPassword($identity, $this->input->post('new'));
    ...
    /**@
    Função para ativar o utilizador
    @return void
    **/
    ...
    **/
    function deactivate($id = null)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->template->load('template', 'auth/deactivate_user', $this->data);
    ...
    **/
    function edit_group($id)
    {
        ...
        //validate form input
        $this->form_validation->set_rules(
            'group_name', $this->lang->line('edit_group_validation_name_label'),
            'required|alpha_dash|xss_clean'
        );
        $this->form_validation->set_rules(
            'group_description',
            $this->lang->line('edit_group_validation_desc_label'), 'xss_clean'
        );

        if (isset($_POST) && !empty($_POST)) {
            if ($this->form_validation->run() === true) {
                $group_update = $this->ion_auth->update_group(
                    $id, $_POST['group_name'], $_POST['group_description']
                );
    ...
    /**
    Função para fazer algo com o csrf_ nonce
    @return void
    **/
    ...
    /**
    Função para validar o csrf_nonce
    @return void
    **/
    ...
    /**
    Função para fazer o render da página
    acho que já não é precisa
    @return void
    **/
